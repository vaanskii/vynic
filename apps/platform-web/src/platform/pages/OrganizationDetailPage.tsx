...
export function OrganizationDetailPage() {
  ...
  return <>
    ...
    <Panel title="Venues" description="Locations owned by this organization.">
      {organization.venues?.length ? <Table><thead><tr><th>Venue</th><th>Status</th><th>Timezone</th><th>Currency</th><th aria-label="Actions" /></tr></thead><tbody>{organization.venues.map((venue) => <tr key={venue.id}><td><div className="platform-table__primary"><strong>{venue.name}</strong><small>{venue.id}</small></div></td><td><StatusBadge value={venue.status} /></td><td>{venue.timezone}</td><td>{venue.currency}</td><td><div className="platform-table__actions"><Link className="platform-text-link" to={`/venues/${venue.id}`}>Open</Link></div></td></tr>)}</tbody></Table> : <EmptyState title="No venues in this organization" body="Create a venue from the Venues directory and assign it to this organization." action={<Link className="platform-text-link" to="/venues">Go to Venues</Link>} />}
    </Panel>
