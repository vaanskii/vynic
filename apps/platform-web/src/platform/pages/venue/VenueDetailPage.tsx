...
export function VenueDetailPage() {
  ...
  const status = useMutation({ mutationFn: () => platformApi.setVenueStatus(venueId, venue.data?.status === "ACTIVE" ? "DISABLED" : "ACTIVE"), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["venue", venueId] }); await queryClient.invalidateQueries({ queryKey: ["venues"] }); setConfirmStatus(false); setFeedback("Venue status updated."); } });
  ...
  return <>
    <PageHeader eyebrow={`${record.organization?.name ?? record.organizationId} / Venue`} title={record.name} description="Platform configuration and infrastructure for this location." actions={<><StatusBadge value={record.status} /><Button onClick={() => { setForm({ name: record.name, timezone: record.timezone, currency: record.currency }); setEditOpen(true); }}><PencilSimple size={16} /> Edit details</Button><Button tone={record.status === "ACTIVE" ? "danger" : "secondary"} onClick={() => setConfirmStatus(true)}><Power size={16} /> {record.status === "ACTIVE" ? "Disable" : "Enable"}</Button></>} />
    <nav className="platform-tabs" aria-label="Venue sections">{tabs.map((name) => <NavLink key={name} end={name === "overview"} className={({ isActive }) => isActive ? "is-active" : ""} to={name === "overview" ? `/admin/venues/${venueId}` : `/admin/venues/${venueId}/${name}`}>{name[0].toUpperCase() + name.slice(1)}</NavLink>)}</nav>
