...
const tabs = ["overview",
  "commercial",
  "product", "website", "devices", "activity",
] as const;
...
export function VenueDetailPage() {
  ...
  const update = useMutation({ mutationFn: () => platformApi.updateVenue(venueId, form), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["venue", venueId] }); await queryClient.invalidateQueries({ queryKey: ["venues"] }); setEditOpen(false); setFeedback("Venue details updated."); },
  });
  const status = useMutation({ mutationFn: () => platformApi.setVenueStatus(venueId, venue.data?.status === "ACTIVE" ? "DISABLED" : "ACTIVE",
      ), onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["venue", venueId] }); await queryClient.invalidateQueries({ queryKey: ["venues"] }); setConfirmStatus(false); setFeedback("Venue status updated."); },
  });

  if (venue.isPending) return <LoadingState label="Loading venue" />;
  if (venue.error) return (
      <ErrorState error={errorMessage(venue.error)} retry={() => void venue.refetch()} />
    );
  const record = venue.data;
  const currentTab = tabs.includes(tab as (typeof tabs)[number]) ? tab : "overview";
  if (currentTab !== tab) return <Navigate to={`/admin/venues/${venueId}`} replace />;
  ...
  return (
    ...
    <nav className="platform-tabs" aria-label="Venue sections">{tabs.map((name) => (
          <NavLink key={name} end={name === "overview"} className={({ isActive }) => (isActive ? "is-active" : "")} to={name === "overview" ? `/admin/venues/${venueId}` : `/admin/venues/${venueId}/${name}`}>{name[0].toUpperCase() + name.slice(1)}</NavLink>))}</nav>
    {feedback ? (
    ...
    {currentTab === "devices" ? (
        <VenueDevicesTab venueId={venueId} venueName={record.name} />
      ) : null}
    {currentTab === "activity" ? (
