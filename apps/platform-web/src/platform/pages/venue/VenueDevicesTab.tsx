...
type DeviceAction = { device: Device; kind: "status" | "rotate" | "primary"; nextStatus?: DeviceStatus };
...
export function VenueDevicesTab({ venueId, venueName }: { venueId: string; venueName: string }) {
  ...
  const mutateDevice = useMutation({ mutationFn: async (action: DeviceAction) => {
    if (action.kind === "primary") return platformApi.selectOperationalPrimary(venueId, action.device.id, action.device.activeOperationalDeviceId ?? null);
  ...
  }, onSuccess: async () => { await refresh(); setDeviceAction(null); setFeedback("Device settings updated."); } });
  ...
  return <>
    ...
    <Panel title="POS devices" description="Only the selected Primary POS may sync restaurant operations. Multi-POS coordination is not enabled yet. Stop the previous POS and verify the replacement data before changing the primary." action={<Button tone="quiet" onClick={() => setCreateOpen(true)}><Plus size={16} /> Issue credential manually</Button>}>
      {devices.data.length > 0 && !devices.data.some((d) => d.isOperationalPrimary) ? <p role="status">Primary POS not selected. Select one to resume operational sync.</p> : null}
      {devices.data.length === 0 ? <EmptyState title="No POS devices" body="Create a device to issue its one-time Edge credential." /> : <Table><thead><tr><th>Device</th><th>Platform</th><th>Status</th><th>Last seen</th><th aria-label="Actions" /></tr></thead><tbody>{devices.data.map((device) => <tr key={device.id}><td><div className="platform-table__primary"><strong>{device.displayName}</strong><small>{device.isOperationalPrimary ? "Primary POS" : "Secondary — operational sync inactive"}</small><small>{device.installationId}</small><small>Created {formatDateTime(device.createdAt)} · Updated {formatDateTime(device.updatedAt)}</small></div></td><td>{device.platform}</td><td><StatusBadge value={device.status} /></td><td><div className="platform-device-activity"><strong>{formatRelativeTime(device.lastSeenAt)}</strong><small>{device.lastSeenAt ? formatDateTime(device.lastSeenAt) : "No connection recorded"}</small></div></td><td><div className="platform-table__actions">{!device.isOperationalPrimary && device.status === "ACTIVE" ? <Button onClick={() => setDeviceAction({ device, kind: "primary" })}>Make Primary POS</Button> : null}<Button tone="quiet" onClick={() => setDeviceAction({ device, kind: "rotate" })}>Rotate credential</Button>{device.status === "ACTIVE" ? <><Button tone="quiet" onClick={() => setDeviceAction({ device, kind: "status", nextStatus: "DISABLED" })}>Disable</Button><Button tone="danger" onClick={() => setDeviceAction({ device, kind: "status", nextStatus: "REVOKED" })}>Revoke</Button></> : <Button tone="secondary" onClick={() => setDeviceAction({ device, kind: "status", nextStatus: "ACTIVE" })}>Activate</Button>}</div></td></tr>)}</tbody></Table>}
    </Panel>
    <Panel title="Send connection test" description="Queues a safe asynchronous NOOP for the venue or one device. A queued command does not prove the POS executed it.">
      <div className="platform-test-command"><Field label="Target"><Select value={testDeviceId} onChange={(event) => { setTestDeviceId(event.target.value); setQueued(null); }}><option value="">Primary POS in {venueName}</option>{devices.data.map((device) => <option key={device.id} value={device.id}>{device.displayName}</option>)}</Select></Field><Button tone="primary" onClick={() => testCommand.mutate()} loading={testCommand.isPending}><Pulse size={16} /> {testCommand.isPending ? "Queueing…" : "Send connection test"}</Button></div>
    ...
    <Dialog open={createOpen} onOpenChange={(open) => { if (!create.isPending) setCreateOpen(open); }} title="Issue a credential manually" description="The support path. It hands a long-lived secret to a person, which enrollment exists to avoid — prefer an enrollment code unless the terminal cannot reach this server." footer={<><Button disabled={create.isPending} onClick={() => setCreateOpen(false)}>Cancel</Button><Button tone="primary" type="submit" form="create-device" loading={create.isPending} loadingLabel="Creating…">Issue credential</Button></>}><form id="create-device" className="platform-form" onSubmit={(event) => { event.preventDefault(); if (!create.isPending) create.mutate(); }}><Field label="Display name"><Input value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} required maxLength={200} autoFocus /></Field><Field label="Platform"><Input value={form.platform} onChange={(event) => setForm({ ...form, platform: event.target.value.toUpperCase() })} required maxLength={32} /></Field><Field label="Installation ID" hint="Optional UUID. Leave empty to generate one automatically."><Input value={form.installationId} onChange={(event) => setForm({ ...form, installationId: event.target.value })} /></Field><FormError>{create.error ? errorMessage(create.error) : undefined}</FormError></form></Dialog>
    ...
    <ConfirmDialog open={Boolean(deviceAction)} onOpenChange={(open) => { if (!open) setDeviceAction(null); }} title={deviceAction?.kind === "primary" ? `Make ${deviceAction.device.displayName} the Primary POS?` : deviceAction?.kind === "rotate" ? `Rotate credential for ${deviceAction.device.displayName}?` : `${deviceAction?.nextStatus === "REVOKED" ? "Revoke" : deviceAction?.nextStatus === "DISABLED" ? "Disable" : "Activate"} ${deviceAction?.device.displayName}?`} description={deviceAction?.kind === "primary" ? "Confirm the previous POS has stopped and this replacement contains the correct restaurant data. The previous primary loses operational sync access. Unfinished commands require reconciliation; data is not copied automatically." : deviceAction?.kind === "rotate" ? "The current POS credential will immediately stop working. The replacement will be shown only once." : deviceAction?.nextStatus === "REVOKED" ? "This is not a delete. The credential stops authenticating immediately and permanently; the device row, its command history and its audit trail all stay. The terminal can only come back by being enrolled again with a new code." : deviceAction?.nextStatus === "DISABLED" ? "The credential will stop authenticating until the device is activated again. No credential is rotated." : "The existing credential may authenticate again immediately. No new credential is issued."} confirmLabel={deviceAction?.kind === "primary" ? "Previous POS stopped — switch primary" : deviceAction?.kind === "rotate" ? "Rotate and issue new credential" : "Change device status"} danger={deviceAction?.kind === "rotate" || deviceAction?.nextStatus === "REVOKED"} pending={mutateDevice.isPending} error={mutateDevice.error ? errorMessage(mutateDevice.error) : undefined} onConfirm={() => { if (deviceAction) mutateDevice.mutate(deviceAction); }} />
  </>;
}
