...
export function VenueProductTab({ venueId }: { venueId: string }) {
  ...
  const override = useMutation({ mutationFn: async (change: PendingOverride) => change.effect === "INHERIT" ? platformApi.clearFeatureOverride(venueId, change.featureKey) : platformApi.setFeatureOverride(venueId, change.featureKey, change.effect, change.note?.trim() || undefined,
          ), onSuccess: async () => { await refresh(); setPendingOverride(null); setFeedback("Feature access updated."); },
  });
  ...
  if (failed) return (
      <ErrorState error={errorMessage(failed)} retry={() => { void product.refetch(); void plans.refetch(); void features.refetch(); }} />
    );
  ...
  return (
    ...
    >
      {state.commercialAccess === false && (
        <p role="status" className="platform-callout">
          Cloud access is suspended or cancelled. Feature entitlements below
          remain assigned; reactivate the subscription to restore access.
        </p>
      )}
      {feedback ? (
        <div className="platform-feedback platform-feedback--success" role="status">{feedback}</div>
      ) : null}
    <div className="platform-grid platform-grid--sidebar">
      <Panel title="Assigned plan" description="The package assigned to this venue.">
        <div className="platform-plan-control"><div><strong>{state.plan?.name ?? "No plan assigned"}</strong><span>{state.planAssignedAt ? `Assigned ${formatDateTime(state.planAssignedAt)}` : "Assign a plan to establish inherited access."}</span></div><div><Select aria-label="Plan" value={selected} onChange={(event) => setSelectedPlanId(event.target.value)}><option value="">Select plan</option>{plans.data.map((plan) => (
                ...
                ))}</Select><Button tone="primary" disabled={!selectedPlanId || selectedPlanId === state.plan?.id} onClick={() => setPlanConfirm(true)}>Change plan</Button></div></div>
      </Panel>
      <Panel title="Effective access" description="Feature entitlements resolved from plan and overrides."
        ><div className="platform-effective-list">{features.data.map((feature) => (
              <div key={feature.key}><span>{feature.name}</span><StatusBadge value={state.effectiveFeatures.includes(feature.key) ? "ENABLED" : "DISABLED"} /></div>
            ))}</div></Panel>
    </div>
    <Panel title="Feature inheritance and overrides" description="Overrides always win. Inherit removes the override and returns authority to the assigned plan.">
      <div className="platform-feature-list">{features.data.map((feature) => {
        ...
        return (
              <article className="platform-feature-row" key={feature.key}><div className={`platform-feature-icon ${effective ? "is-enabled" : ""}`}>{effective ? (
                    <CheckCircle size={21} weight="duotone" />
                  ) : (
                    <MinusCircle size={21} weight="duotone" />
                  )}</div><div className="platform-feature-row__copy"><strong>{feature.name}</strong><span className="platform-mono">{feature.key}</span><p>{source}</p><Input aria-label={`${feature.name} override note`} placeholder="Optional operator note" value={notes[feature.key] ?? existing?.note ?? ""} onChange={(event) => setNotes((current) => ({ ...current, [feature.key]: event.target.value,
                      }))} maxLength={500} /></div><div className="platform-feature-state"><StatusBadge value={effective ? "ENABLED" : "DISABLED"} /><small>{existing?.updatedAt ? `Changed ${formatDateTime(existing.updatedAt)}` : "No explicit exception"}</small></div><div className="platform-feature-actions"><Button tone={existing?.effect === "ENABLED" ? "primary" : "quiet"} onClick={() => setPendingOverride({ featureKey: feature.key, featureName: feature.name, effect: "ENABLED", note: notes[feature.key] ?? existing?.note ?? "",
                      })}>Enable</Button><Button tone={existing?.effect === "DISABLED" ? "danger" : "quiet"} onClick={() => setPendingOverride({ featureKey: feature.key, featureName: feature.name, effect: "DISABLED", note: notes[feature.key] ?? existing?.note ?? "",
                      })}>Disable</Button><Button tone={!existing ? "secondary" : "quiet"} disabled={!existing} onClick={() => setPendingOverride({ featureKey: feature.key, featureName: feature.name, effect: "INHERIT",
                      })}>Inherit</Button></div></article>
            );
      })}</div>
    </Panel>
    {!state.website.consistent ? (
        <div className="platform-callout platform-product-warning"><WarningCircle size={19} /><div><strong>Website configuration is inconsistent</strong><p>The configured website mode and WEBSITE entitlement disagree. Review the Website tab; the frontend has not changed either value automatically.</p></div></div>
      ) : null}
