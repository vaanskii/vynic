...
type PendingOverride = { featureKey: string; featureName: string; effect: OverrideEffect | "INHERIT" };
...
export function VenueProductTab({ venueId }: { venueId: string }) {
  ...
  const assignPlan = useMutation({ mutationFn: () => platformApi.assignPlan(venueId, selectedPlanId), onSuccess: async () => { await refresh(); setPlanConfirm(false); } });
  const override = useMutation({ mutationFn: async (change: PendingOverride) => change.effect === "INHERIT" ? platformApi.clearFeatureOverride(venueId, change.featureKey) : platformApi.setFeatureOverride(venueId, change.featureKey, change.effect), onSuccess: async () => { await refresh(); setPendingOverride(null); } });
  ...
  return <>
    <div className="platform-grid platform-grid--sidebar">
      <Panel title="Assigned plan" description="Commercial package assignment. The backend remains authoritative for effective access.">
      ...
      <Panel title="Effective access" description="Resolved by the same service production feature guards use."><div className="platform-effective-list">{features.data.map((feature) => <div key={feature.key}><span>{feature.name}</span><StatusBadge value={state.effectiveFeatures.includes(feature.key) ? "ENABLED" : "DISABLED"} /></div>)}</div></Panel>
    </div>
    <Panel title="Feature inheritance and overrides" description="Overrides always win. Inherit removes the override and returns authority to the assigned plan.">
      <div className="platform-feature-list">{features.data.map((feature) => {
        ...
        return <article className="platform-feature-row" key={feature.key}><div className={`platform-feature-icon ${effective ? "is-enabled" : ""}`}>{effective ? <CheckCircle size={21} weight="duotone" /> : <MinusCircle size={21} weight="duotone" />}</div><div className="platform-feature-row__copy"><strong>{feature.name}</strong><span className="platform-mono">{feature.key}</span><p>{source}{existing?.note ? ` · ${existing.note}` : ""}</p></div><StatusBadge value={effective ? "ENABLED" : "DISABLED"} /><div className="platform-feature-actions"><Button tone={existing?.effect === "ENABLED" ? "primary" : "quiet"} onClick={() => setPendingOverride({ featureKey: feature.key, featureName: feature.name, effect: "ENABLED" })}>Enable</Button><Button tone={existing?.effect === "DISABLED" ? "danger" : "quiet"} onClick={() => setPendingOverride({ featureKey: feature.key, featureName: feature.name, effect: "DISABLED" })}>Disable</Button><Button tone={!existing ? "secondary" : "quiet"} disabled={!existing} onClick={() => setPendingOverride({ featureKey: feature.key, featureName: feature.name, effect: "INHERIT" })}>Inherit</Button></div></article>;
      })}</div>
    </Panel>
