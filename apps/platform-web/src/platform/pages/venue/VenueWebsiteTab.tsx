...
export function VenueWebsiteTab({ venueId }: { venueId: string }) {
  ...
  const setModeMutation = useMutation({ mutationFn: () => platformApi.setWebsiteMode(venueId, mode as WebsiteMode), onSuccess: async () => { await refreshProduct(); setModeConfirm(false); setMode(""); } });
  const register = useMutation({ mutationFn: () => platformApi.registerDomain(venueId, hostname), onSuccess: async () => { await refreshDomains(); setDomainOpen(false); setHostname(""); } });
  const mutateDomain = useMutation({ mutationFn: async (action: DomainAction) => action.kind === "release" ? platformApi.releaseDomain(venueId, action.domain.id) : platformApi.setDomainStatus(venueId, action.domain.id, action.nextStatus as DomainStatus), onSuccess: async () => { await refreshDomains(); setDomainAction(null); } });
  ...
  return <>
    {!website.consistent ? <div className="platform-callout"><WarningCircle size={20} /><div><strong>Website entitlement and configuration disagree</strong><p>Configured mode is {website.configuredMode}, but effective mode is {website.effectiveMode}. The frontend is showing the inconsistency without rewriting it.</p></div></div> : null}
    ...
    <Panel title="Domains" description="Hostnames are normalized and reserved by the backend. DNS, TLS, registrar, and Cloudflare controls are outside this phase." action={<Button tone="primary" onClick={() => setDomainOpen(true)}><Plus size={16} /> Register domain</Button>}>
    ...
    <ConfirmDialog open={modeConfirm} onOpenChange={setModeConfirm} title={`Change website mode to ${selectedMode}?`} description="This updates only the configured mode. If the WEBSITE entitlement disagrees, the backend will continue to report the inconsistency and effective mode independently." confirmLabel="Change mode" pending={setModeMutation.isPending} onConfirm={() => setModeMutation.mutate()} />
