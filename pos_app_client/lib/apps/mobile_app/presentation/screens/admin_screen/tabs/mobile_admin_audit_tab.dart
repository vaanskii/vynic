...
class _AuditTabState extends State<_AuditTab>
  ...
  int _count(AuditReportStatus s) =>
  ...
  Widget _buildStats() => Padding(
    ...
    child: Row(
      children: [
        _statChip(
          'აქტიური',
          _count(AuditReportStatus.open),
          const Color(0xFFF59E0B),
        ),
        SizedBox(width: 8),
        _statChip(
          'დახურული',
          _count(AuditReportStatus.closed),
          const Color(0xFF10B981),
        ),
        SizedBox(width: 8),
        _statChip(
          'გაუქმებული',
          _count(AuditReportStatus.cancelled),
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              _chip(
                'გახსნა',
                '${report.openedByName} • ${_fmtTs(report.openedAt)}',
              ),
              _chip('განახლება', _fmtTs(report.updatedAt)),
              _chip('მოქმედებები', '${events.length}'),
              if (report.closedAt != null)
                _chip('დახურვა', _fmtTs(report.closedAt!)),
              if (report.closedByName != null)
                _chip('დახურა', report.closedByName!),
              if (lastEvent != null)
                _chip(
                  'ბოლო',
                  '${_eventLabel(lastEvent.type)} • ${lastEvent.waiterName}',
                ),
            ],
          ),
          SizedBox(height: 8),
            ...
            children: [
              Row(
                children: [
                  ...
                  IconButton(
                    onPressed: () =>
                        MobileAdminScreen.viewCheckPdf(context, report.orderId),
                    icon: Icon(
              ...
              Wrap(
                ...
                children: [
                  ...
                  if (report.closedAt != null)
                    _chip('Closed at', _fmtTs(report.closedAt!)),
                  if (report.closedByName != null)
                    _chip(
                      'Closed by',
