...
# windows` in apps/operations must never pull the signing tool in, or a customer
# ends up with a copy of the thing that makes their locks.
#
#   ./tools/build_unlocker.sh              # build for this machine
#   ./tools/build_unlocker.sh windows      # build the Windows executable
#   ./tools/build_unlocker.sh macos
#
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_DIR="$REPO_ROOT/apps/devtool"
